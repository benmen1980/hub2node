...
router.get('/',function(req,res){
    res.send('This service works only with POST method, Make sure to post credentials in the below format(the FORMAT is optioanl): <br>' + '"CPROFNUM":"IV123456","FORMAT":"-7,"' + JSON.stringify( helper.getCredentails()));
});
...
async function webSDK(req) {
    try {
       ...
       let procInput = procedure.input;
        ...
        var data = {EditFields: [
                {field: 1, op: 0, value: req.body.CPROFNUM},
        ...
        try {
            if(url){
                ...
                s3Url = await s3Service.uploadPdfFromUrl(url, `PriceQuote_${req.body.CPROFNUM}_${Date.now()}`, 'pdfs', cookies);
            }
        } catch (e) {
